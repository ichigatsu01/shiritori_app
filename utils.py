...
class Game:
    def __init__(self):
        # この辺app.pyで設定しないとダメな気もする、大丈夫な気もする
        self.worddict_cpu = {} # CPUが使う辞書の読込
    ...
    def check_inputed_user(self, inputed, used, cpu):
    ...
    def choice_word_cpu(self, inputed, dict_cpu, dict_used): # CPUが単語を選ぶ時の処理
        if inputed == "": # cpu先攻の場合は辞書からランダムに選ぶ
            ...
            letter = self.lastletterChecker(inputed, inputed)
...
while gameend_flag == False:
    ...
    if game_start.lower() == 'y':
        while True:
            inputed_word_user = check_inputed_user() #! 要調整：ブラウザで入力した内容を反映させる
